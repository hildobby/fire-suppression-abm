...
for var in list_var:
    ...
    for i in range(1, 4):
        globals()[
            ...
            "data_" +
            str(i))[
            "ofat_{}_{}___repli_{}__dist_samp_{}" .format(
                i,
                var,
                rep,
                dist)].groupby(var).mean().reset_index()[var]

        param = "On Fire"
        globals()[
            'y_{}_on_fire_%s'.format(var) %
            i] = eval(
            "data_" +
            str(i))[
            "ofat_{}_{}___repli_{}__dist_samp_{}" .format(
        ...
        ax[2].fill_between(
            ...
            globals()[
                ('y_{}_extinguish_%s'.format(var) %
                 i)] -
            globals()[
                'err_{}_extinguish_%s'.format(var) %
                i],
            globals()[
                ('y_{}_extinguish_%s'.format(var) %
                 i)] +
